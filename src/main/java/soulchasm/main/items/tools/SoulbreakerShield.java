...
public class SoulbreakerShield extends ShieldTrinketItem {
    public SoulbreakerShield() {
        super(Rarity.EPIC, 5, 0.15F, 10000, 0.3F, 50, 180, 1400, IncursionTrinketsLootTable.incursionTrinkets);
    }
    ...
    public void onShieldHit(InventoryItem item, Mob mob, MobWasHitEvent hitEvent) {
        ...
        if(attacker != null && mob.getLevel().isServer() && attacker instanceof Mob || attacker instanceof Projectile){
            ...
            for(int i = -1; i<=1; i++){
                ...
                SoulFlamethrowerProjectile projectile = new SoulFlamethrowerProjectile(level, mob.x, mob.y, ((Entity) attacker).x, ((Entity) attacker).y, 250, new GameDamage((hitEvent.damage + 60) * 3).modDamage(0.33F), mob);
