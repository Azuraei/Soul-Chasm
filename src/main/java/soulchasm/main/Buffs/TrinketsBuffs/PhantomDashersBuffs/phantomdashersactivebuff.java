...
public class phantomdashersactivebuff extends Buff implements MovementTickBuff {
    ...
    public void clientTick(ActiveBuff buff) {
        ...
        if ((owner.dx != 0.0F || owner.dy != 0.0F) && !owner.isRiding() && owner.isPlayer) {
            AfterimageParticle particle = new AfterimageParticle(owner.getLevel(), (PlayerMob)owner);
            owner.getLevel().entityManager.addParticle(particle, Particle.GType.COSMETIC);
        }
    }
