...
public class SouldSwordProjectile extends Projectile {
    public float fadeInAndOutMod;

    public SouldSwordProjectile(){

    }

    public SouldSwordProjectile(Level level, float x, float y, float targetX, float targetY, float targetSpeed, int distance, GameDamage damage, int knockback, Mob owner) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        if (!this.removed()) {
            ...
            final TextureDrawOptions options = SouldSwordProjectile.this.texture.initDraw().light(light.minLevelCopy(Math.min(light.getLevel() + 100.0F, 150.0F))).rotate(this.getAngle() + 45.0F, 2, 2).pos(drawX, drawY - (int)this.getHeight()).alpha(fadeInAndOutMod);
