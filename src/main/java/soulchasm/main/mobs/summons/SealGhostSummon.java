...
import soulchasm.main.misc.PlayerFlyingFollowerShooterAI;
import soulchasm.main.projectiles.SoulHomingProjectile;

import java.awt.*;
import java.util.List;

public class SealGhostSummon extends FlyingAttackingFollowingMob {
    public static GameTexture texture;
    public SealGhostSummon() {
        super(10);
    ...
    public void init() {
        ...
        this.ai = new BehaviourTreeAI(this, new PlayerFlyingFollowerShooterAI(576, CooldownAttackTargetAINode.CooldownTimer.TICK, 600, 500, 600, 5) {
            protected boolean shootAtTarget(Mob mob, Mob target) {
                float angle = 15;
                for (int i = -1; i <= 1; ++i) {
                    SoulHomingProjectile entity = new SoulHomingProjectile(mob.getLevel(), mob, mob.x, mob.y + 20, target.x, target.y, 150, 1200, SealGhostSummon.this.summonDamage, 75);
