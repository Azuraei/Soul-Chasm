...
public class ChasmMageStatue extends HostileMob implements OEVicinityBuff {
    ...
    public void init() {
        super.init();
        StationaryPlayerShooterAI<ChasmMageStatue> mobAI = new StationaryPlayerShooterAI<ChasmMageStatue>(400) {
            public boolean canAttack(ChasmMageStatue mob) {
                boolean canAttack = super.canAttack(mob);
                boolean buffingMob = mob.isBuffingMob;
                return canAttack && !buffingMob;
            }
            public void shootTarget(ChasmMageStatue mob, Mob target) {
                for(int i = -1; i<=1; i++){
                    ...
                    ChasmMageStatue.this.attack((int) (mob.x + projectile.dx * 100.0F), (int) (mob.y + projectile.dy * 100.0F), true);
                    ChasmMageStatue.this.getLevel().entityManager.projectiles.add(projectile);
                }
            }
        };
    ...
    public Predicate<Mob> buffMobsFilter() {
        return (m) -> !m.isBoss() && m.isHostile && !m.isSummoned && !(m instanceof ChasmMageStatue);
    }
